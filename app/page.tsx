  ...
  if (overlay === 'connect') {
    return (
      <motion.section
        ...
        id={id}
        className={cn(sectionClasses, "transition-all duration-300")}
                ...
                {[
                  ...
                  },
                  {
                    href: INSTAGRAM_PAGE_URL, label: "Instagram", IconSvg: (
                      <svg viewBox="0 0 448 512" fill="currentColor" className="h-6 w-6 text-white mx-auto"><path d="M224.1 141c-63.6 0-114.9 51.3-114.9 114.9s51.3 114.9 114.9 114.9S339 319.5 339 255.9 287.7 141 224.1 141zm0 189.6c-41.1 0-74.7-33.5-74.7-74.7s33.5-74.7 74.7-74.7 74.7 33.5 74.7 74.7-33.6 74.7-74.7 74.7zm146.4-194.3c0 14.9-12 26.8-26.8 26.8-14.9 0-26.8-12-26.8-26.8s12-26.8 26.8-26.8 26.8 12 26.8 26.8zm76.1 27.2c-1.7-35.9-9.9-67.7-36.2-93.9-26.2-26.2-58-34.4-93.9-36.2-37-2.1-147.9-2.1-184.9 0-35.8 1.7-67.6 9.9-93.9 36.1s-34.4 58-36.2 93.9c-2.1 37-2.1 147.9 0 184.9 1.7 35.9 9.9 67.7 36.2 93.9s58 34.4 93.9 36.2c37 2.1 147.9 2.1 184.9 0 35.9-1.7 67.7-9.9 93.9-36.2 26.2-26.2 34.4-58 36.2-93.9 2.1-37 2.1-147.8 0-184.8zM398.8 388c-7.8 19.6-22.9 34.7-42.6 42.6-29.5 11.7-99.5 9-132.1 9s-102.7 2.6-132.1-9c-19.6-7.8-34.7-22.9-42.6-42.6-11.7-29.5-9-99.5-9-132.1s-2.6-102.7 9-132.1c7.8-19.6 22.9-34.7 42.6-42.6 29.5-11.7 99.5-9 132.1-9s102.7-2.6 132.1 9c19.6 7.8 34.7 22.9 42.6 42.6 11.7 29.5 9 99.5 9 132.1s2.7 102.7-9 132.1z"/></svg>
                    )
                  },
                  {
                    href: "https://linktr.ee/splendidbeautybarandco", label: "Linktree", IconSvg: (
                      <svg viewBox="0 0 24 24" fill="currentColor" className="h-6 w-6 text-white mx-auto"><path d="m13.736 5.853 4.005-4.117 2.325 2.38-4.2 4.005h5.908v3.305h-5.937l4.229 4.108-2.325 2.334-5.74-5.769-5.741 5.769-2.325-2.325 4.229-4.108H2.226V8.121h5.909l-4.2-4.004 2.324-2.381 4.005 4.117V0h3.472zm-3.472 10.306h3.472V24h-3.472z"/></svg>
                    )
                  },
                  {
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Track current section for video preloading
      const sectionHeight = window.innerHeight;
      const currentIndex = Math.floor(el.scrollTop / sectionHeight);
  ...
  // Prepare video paths for preloading
  const videoPaths = useMemo(() => {
    return sectionsData
      .filter(section => {
        const config = getSectionConfig(section.id);
        return config && config.videoName;
      })
      .map(section => {
        const config = getSectionConfig(section.id)!;
        return `/images/optimized/${config.videoName}_optimized.mp4`;
      });
  }, []);
  
  // Get videos to preload based on current position
  const videosToPreload = useMemo(() => {
    const startIndex = Math.max(0, currentSectionIndex);
    const endIndex = Math.min(videoPaths.length, startIndex + 3); // Preload next 3 videos
    return videoPaths.slice(startIndex, endIndex);
  }, [currentSectionIndex, videoPaths]);
  ...
  return (
    <ErrorBoundary fallback={
      ...
      {/* Video Preloader for performance */}
      <VideoPreloader videoPaths={videosToPreload} priority="high" />
